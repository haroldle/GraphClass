...
class GraphSageModule(eqx.Module):
    pool_linear: eqx.nn.Conv2d
    sage_linear: eqx.nn.Conv2d
    pool_normalize: eqx.nn.GroupNorm
    sage_normalize: eqx.nn.GroupNorm
    ...
    def __init__(self, in_channels, out_channels, groups, leaky_relu_alpha, key):
        ...
        self.pool_linear = eqx.nn.Conv2d(
        ...
        self.sage_linear = eqx.nn.Conv2d(
        ...
        self.pool_normalize = eqx.nn.GroupNorm(
            groups=groups,
            channels=in_channels
        )

        self.sage_normalize = eqx.nn.GroupNorm(
            groups=groups,
    ...
    def _init_weights(self, key_: jax.Array):
        ...
        new_weights_pool_linear = jax.nn.initializers.he_normal()(
            key_,
            self.pool_linear.weight.shape,
            dtype=jnp.float32
        )

        self.pool_linear = eqx.tree_at(
            where,
            self.pool_linear,
            new_weights_pool_linear
        )

        new_weights_sage_linear = jax.nn.initializers.he_normal()(
            key_,
            self.sage_linear.weight.shape,
            dtype=jnp.float32
        )

        self.sage_linear = eqx.tree_at(
            where,
            self.sage_linear,
            new_weights_sage_linear
        )
    ...
    def __call__(self, x):
        ...
        # AGGREGATE POOLING
        x_knn = self.leaky_relu(self.pool_normalize(self.pool_linear(x_knn)), self.leaky_relu_alpha)
        x_ = jnp.amax(
            x_knn,
            axis=-1,
            keepdims=True
...
def test(
    ...
    model_loaded = eqx.tree_deserialise_leaves(
        '/home/thanhle/SOTA/LocNet_GraphSage_dynamic_86_k_100_SOTA.eqx',
        model
    )
    ...
    for epoch in range(EPOCHS):
        ...
        if dist < dist_loss:
            ...
            eqx.tree_serialise_leaves(
                f"/home/UNT/tdl0134/SOTA/LocNet_GraphSage_dynamic_{epoch + 1}.eqx",
                model
            )
...
if __name__ == "__main__":
    ...
    # Train = RadioMapDataset('/home/UNT/tdl0134', 'Train_0_0001_TO_0_001')
    # Val = RadioMapDataset('/home/UNT/tdl0134', 'Val_0_0001_TO_0_001')

...
def get_graph_feature_jax(
    ...
    graph_feature_from_original = jnp.tile(
        x_feature_maps.transpose(1, 0).reshape(all_pixels, 1, n_dims),
        (1, 20, 1)
    )
    # Concatenate by channel
    # from row*col, k, channel => row*col, k, 2*channel
    # return a 2*channels, row*col, k
    return jnp.concatenate(
        [graph_feature_map - graph_feature_from_original, graph_feature_from_original],
        axis=2
    ).transpose(2, 0, 1)


class EdgeConvModule(eqx.Module):
    conv: eqx.nn.Conv2d
    normalize: eqx.nn.GroupNorm
    leaky_relu_alpha: float
    leaky_relu: jax.nn.leaky_relu

    def __init__(self, in_channels, out_channels, groups, leaky_relu_alpha, key):
        weight_init_key, conv_key = jax.random.split(key, 1 + 1)
        ...
        self.normalize = eqx.nn.GroupNorm(
            groups=groups,
    ...
    def __call__(self, x):
        x = get_graph_feature_jax(x)
        x = self.leaky_relu(self.normalize(self.conv(x)), self.leaky_relu_alpha)
        return jnp.amax(x, axis=-1, keepdims=False)
...
def test(
    ...
    model_loaded = eqx.tree_deserialise_leaves(
        '/home/thanhle/SOTA/LocNet__edge_conv_k_100_graph_dynamic_75.eqx',
        model
    )
    ...
    for epoch in range(EPOCHS):
        ...
        if dist < dist_loss:
            ...
            eqx.tree_serialise_leaves(
                f"/home/thanhle/SOTA/LocNet_graph_dynamic_{epoch + 1}.eqx",
                model
            )
